...
def get_prop_data(filename, with_autocorr=True):
    ...
    if with_autocorr:
        ...
        i = 0
    autocorr = prop_data[:, i+1]
    X, Y, Z = prop_data[:, i+2], prop_data[:, i+3], prop_data[:, i+4]
    energy = prop_data[:, i+5]
    # Fx, Fy, Fz = prop_data[:, i+6], prop_data[:, i+7], prop_data[:, i+8]
    dipoles = (X, Y, Z)
    # fields = (Fx, Fy, Fz)
...
def plot_hhg(freq, G_obs, G_obs_vel, G_obs_acc, axes, label, xmax=15, w0=0.056961578478002):
    ...
    axes[0].plot(ho, hhg, linewidth=1.0, color='red', label=label +
                 r': $\frac{1}{|t_{f}- t_{i}|}\int \langle {D_{z}}(t)\rangle e^{i\omega t} dt$')
    axes[0].legend(frameon=False, loc='upper right')
    axes[0].set_yscale('log')
    axes[0].set_xlim(0, xmax)
    axes[0].tick_params(labelbottom=False)
    axes[1].plot(ho_vel, hhg_vel, linewidth=1.0, color='blue', label=label +
                 r': $\frac{1}{|t_{f}- t_{i}|}\int \langle \dot{D_{z}}(t)\rangle e^{i\omega t} dt$')
    axes[1].legend(frameon=False, loc='upper right')
    axes[1].set_yscale('log')
    axes[1].set_xlim(0, xmax)
    axes[1].tick_params(labelbottom=False)
    axes[1].set_ylabel(r'Signal in (a.u.)', size=18)
    axes[2].plot(ho_acc, hhg_acc, linewidth=1.0, color='orange', label=label +
                 r': $\frac{1}{|t_{f}- t_{i}|}\int \langle \ddot{D_{z}}(t)\rangle e^{i\omega t} dt$')
